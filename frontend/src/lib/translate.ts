...
const REPLACEMENTS: Replacement[] = [
  ...
  // Additional splitting narrative patterns
  [/\b(\d+) ordenes de compra\b/gi, "$1 purchase orders"],
  [/\bordenes de compra\b/gi, "purchase orders"],
  [/\borden de compra\b/gi, "purchase order"],
  [/\bentre (\d{4}-\d{2}-\d{2}) y (\d{4}-\d{2}-\d{2})\b/gi, "between $1 and $2"],
  [/\bcada una debajo de\b/gi, "each below"],
  [/\bcada uno debajo de\b/gi, "each below"],
  [/\bsumando\b/gi, "totaling"],
  [/\btodas firmadas por\b/gi, "all signed by"],
  [/\btodas aprobadas por\b/gi, "all approved by"],
  [/\btodos firmados por\b/gi, "all signed by"],
  [/\brecibi[oó] (\d+) ordenes\b/gi, "received $1 orders"],
  [/\bdesviacion\b/gi, "deviation"],
  [/\bdesviaci[oó]n\b/gi, "deviation"],
  [/\barriba del (\d+)% permitido\b/gi, "above the $1% tolerance"],
  [/\bel efecto del 69-B\b/gi, "the 69-B effect"],
  [/\balcanza retroactivamente\b/gi, "reaches retroactively"],
  [/\bel SAT\b/g, "SAT"],
  [/\bpol[ií]tica interna\b/gi, "internal policy"],
  [/\balta ([\d-]+)/gi, "registered $1"],
  [/\bCLABE\b/g, "CLABE"],
  [/\btodas\b/gi, "all"],
  [/\btodos\b/gi, "all"],
  [/\bpero la suma\b/gi, "but the sum"],
  [/\bde facturas del mismo mes es\b/gi, "of invoices for the same month is"],
  [/\by no hay ninguna\b/gi, "and there is no"],
  [/\bde ese proveedor a la empresa en el mismo mes\b/gi, "from that vendor to the company in the same month"],
  [/\ben (\d) pago\(s\)\b/gi, "in $1 payment(s)"],
  [/\ben (\d) factura\(s\)\b/gi, "in $1 invoice(s)"],
];
